...
class Model():
    def __init__(self, conf):
        ...
        cell_fw = tf.nn.rnn_cell.BasicLSTMCell(self.conf.rnn_size)
        cell_bw = tf.nn.rnn_cell.BasicLSTMCell(self.conf.rnn_size)
        
        if conf.keep_prob < 1.0 and not conf.infer:
            cell_fw = tf.nn.rnn_cell.DropoutWrapper(cell_fw, output_keep_prob=conf.keep_prob)
            cell_bw = tf.nn.rnn_cell.DropoutWrapper(cell_bw, output_keep_prob=conf.keep_prob)
        self.cell_fw = cell_fw = tf.nn.rnn_cell.MultiRNNCell([cell_fw] * self.conf.num_layers)
        self.cell_bw = cell_bw = tf.nn.rnn_cell.MultiRNNCell([cell_bw] * self.conf.num_layers)
        ...
        outputs_bi = tf.nn.bidirectional_rnn(cell_fw, cell_bw, inputs, initial_state_fw=self.initial_state_fw, initial_state_bw=self.initial_state_bw, scope='rnn')
        #print outputs_bi
        output = tf.reshape(tf.concat(1, outputs_bi[0]), [-1, self.conf.rnn_size*2])
